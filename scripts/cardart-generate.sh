...
# 측면(facet)마다 굽지 않는 이유: content.ts 실측상 측면 55개가 **태그 조합 7종**으로
# 접히고 그중 5종이 55개 중 53개를 덮는다. 구우면 ≈150장, 계산하면 49장 + 처리 5종.
# 게다가 계산본은 **측면이 확정되는 순간 전이로 움직인다** — 12의 "배치=확정"이 그림이 된다.
#
# ── 생성 규칙 3가지 (전부 실측으로 얻음) ────────────────────────────────────
# ① 용도가 아니라 사물을 그린다.
#    12 §1: 카드의 의미는 이웃이 만든다. 아트가 한 측면에 커밋하면 "다면성 발견"이
#    죽는다. 실·섬유를 "빗장을 당기는 실"로 그리면 route 측면을 미리 확정해버린다.
# ② 바탕을 평평한 근-흑색으로 못박는다.
