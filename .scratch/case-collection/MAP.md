...
- [메커닉 설계 공간 서베이](issues/01-mechanic-survey.md) — 권고: 수집=퍼즐재료(Golden Idol형)로 MVP 후 단서-투-덱(×StS)으로 확장; Balatro형 점수엔진은 마스터리 최고나 내러티브 결합 최약, 도감은 리텐션 레이어; 최대 과제는 리플레이성.
...
- [최신 추상전략게임 메커닉 리서치](issues/15-abstract-games-research.md) — 핵심: "정답 맞히기+시나리오 조립" 통합은 **Golden Idol(2022)식 빈칸 두루마리**로 이미 검증됨(caseCollection 추리문이 이미 그 형태). 톱 추천=①빈칸 두루마리를 통합 척추로(v4 시나리오 보드를 추리문 자체로 흡수) ②인접 시너지(Azul) ③제약 만족 재확인(Obra Dinn/CSP) ④문맥 태그=영역 다수결(Tigris→12) ⑤템포(Dune)로 인터루드 추상화. 통합=봉합 아님, 두루마리 위에 층층이 쌓기.
...
- 인터루드 이벤트·BAD 엔딩의 콘텐츠 생성 — 이벤트 텍스트·연출을 빌드타임에 어떻게 만들지. 문맥 태그 의미론 설계(12) 확정 후 구체화.
- case 생성 파이프라인의 형태 — 패턴×story 소재풀 조합과 배경 상태 규칙을 만드는 빌드타임 생성기. 코어 루프 확정으로 게임 필수 요구가 됨 — 카드 스키마·태그 의미론 확정 후 티켓화 (06에서 OUT의 `episode_generator.py`가 참고 원형으로 확인됨 — 게임측 재구현).
