...
- case 생성 파이프라인의 형태 — 패턴×story 소재풀 조합과 배경 상태 규칙을 만드는 빌드타임 생성기. 코어 루프 확정으로 게임 필수 요구가 됨 — 카드 스키마·태그 의미론 확정 후 티켓화 (06에서 OUT의 `episode_generator.py`가 참고 원형으로 확인됨 — 게임측 재구현). **03 재개정으로 요건 확정: 생성물은 "①풀 수 있음(어휘 게이트 솔버빌리티·소프트락 없음) + ②정답이 이야기로 성립함(정답 슬롯 순서 = 서사 인과 순서)"의 이중 제약을 만족해야 하며, 슬롯마다 `frame`/`accepts`를 출력해야 한다.**
