...
- 2026-08-02: 사용자가 현재 MVP·NAN 2026 제출 흐름에서는 이 티켓을 진행하지 않고 이후로 유예하기로 결정했다. 승인된 host-driven compiler MCP 방향은 보존하되, MVP 밖 제품 스코프를 명시적으로 다시 확장하기 전에는 후속 설계·구현에 착수하지 않는다. Status는 open으로 유지한다.
