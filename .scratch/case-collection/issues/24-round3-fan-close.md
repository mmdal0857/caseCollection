...
핸드 슈트 탭을 눌러 펼침(`.stack-fan`)이 열린 뒤, **카드를 고르고 얼굴을 선택해 배치를 끝내도 펼침이 닫히지 않는다.** `.stack-fan`은 `position: fixed; bottom: 174px; min-height: 230px`이므로 열려 있는 동안 **반응 띠(`.reaction-band`)와 핸드(`.hand-rail`)을 덮는다.**
...
근거는 [20](20-play-screen-hierarchy.md)의 위계다 — **핸드는 "대기 층"**이다. 펼침의 목적은 *고르기* 하나이고, 고른 뒤의 무대는 두루마리(주인공)와 반응 띠다. 배치가 끝날 때까지 열어 두면 대기 층이 주인공 층을 가린다.
