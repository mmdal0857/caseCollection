...
핸드 슈트 탭을 눌러 펼침(`.stack-fan`)이 열린 뒤, **카드를 고르고 측면을 선택해 배치를 끝내도 펼침이 닫히지 않는다.** `.stack-fan`은 `position: fixed; bottom: 174px; min-height: 230px`이므로 열려 있는 동안 **반응 띠(`.reaction-band`)와 핸드(`.hand-rail`)을 덮는다.**
...
즉 닫히는 시점은 "배치 완료"가 아니라 **"선택 완료"**다. 측면 피커가 열리는 동안에도 펼침이 떠 있으면 안 된다.
