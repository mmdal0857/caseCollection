...
4. 사용자 요청에 따라 공개 플레이스루 시작 전에 오디오 UI의 `전체 음소거`를 켰고, Home부터 컬렉션·데이터 팩 화면까지 `음소거` 표시가 유지됐다. 공개 오디오 매니페스트는 200이었으며, 같은 `de8bdb3` 로컬 production 빌드에서는 음악·효과음과 배경·카드 자산 요청이 모두 200/304였다. 승인 replay에서 다시 만든 `game-data-pack@2`(output `66791aea…`, provenance `2a3e4a2b…`)는 로컬 production 개발자 UI에서 추가 46·상쇄 0·문제 0으로 적용되고 새로고침 뒤 `generated.pg204-invisible-man.json`이 IndexedDB에서 재수화됐다. 공개 배포본에서도 데이터 팩 UI가 추가 44·상쇄 0·문제 0과 `검증된 팩 적용`을 표시해 해당 진입점이 배선됐음을 확인했다.
5. 배포본의 1024×768·1280×720 브라우저 창에서 document overflow 0, 플레이 보드 하단 오차 0px, 반응 띠–핸드 간격 16px, 반응 띠–핸드 겹침 0, 오디오 UI–핸드 겹침 0을 실측했다. 새 음소거 공개 세션의 브라우저 warning/error 로그는 0건이었고, document·레이든 WebP·오디오 매니페스트 HTTP 확인도 모두 200이었다.
