...
**조건 해소 (2026-07-28)**: 사용자가 **격리 유지**를 결정했다("일단 유지"). 아래 배치는 지도 26행에 **`/ 유예`로 기록**해 해소했다 — 오버라이드가 절대적이지 않다는 예외가 명시됐으므로 이 티켓의 검토는 **조건 없는 수용**이 된다. 다만 ⓓ의 주의는 그대로 살아 있다: 이 티켓의 검증 결과는 브랜치에서의 것이고 main에서 재현되지 않는다.
