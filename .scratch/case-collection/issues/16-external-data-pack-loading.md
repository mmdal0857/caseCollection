...
- 2026-07-23 (fable session) **팩 계약의 프로토판 랜딩** (근거 자산 — 계약 확정은 여전히 이 티켓 소관):
  ...
  - 실측 ②: **id 충돌은 기능이자 함정** — 위키 추출 49건 중 2건(`omitted_witness` 등)이 수제 base id와 동명이라, 접두사 없는 병합이 base 카드를 덮어써 c3·boss가 풀 수 없게 됨 — 무결성 검사가 "정답 카드에 해당 얼굴 없음"으로 검출. 정식 계약은 **병기(add-alongside) vs 승격(promotion 상쇄)을 명시적 선택**으로 둬야 한다는 근거.
